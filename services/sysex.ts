...
export class SysExHandler {
  static parseFile(buffer: ArrayBuffer): Patch[] {
    ...
    if (data[0] === 0xF0 && data[1] === 0x43 && (data[3] & 0x0F) === 0x00) {
      if (data.length >= 161) {
        const dataStart = data.length === 163 ? 6 : 4;
        return [this.unpackUnpackedVoice(data.slice(dataStart, dataStart + 155))];
      }
    }
  ...
  private static unpackUnpackedVoice(d: Uint8Array): Patch {
    ...
    const name = new TextDecoder().decode(d.slice(145, 155)).replace(/[^\x20-\x7E]/g, '').trim() || "INIT VOICE";
    return {
      name,
      algorithm: (d[134] & 31) + 1,
      feedback: d[135] & 7,
      oscKeySync: !!d[136],
      operators,
      lfoSpeed: d[137],
      lfoDelay: d[138],
      lfoPitchModDepth: d[139],
      lfoAmpModDepth: d[140],
      lfoSync: !!d[141],
      lfoWaveform: d[142] & 7,
      lfoPitchModSens: d[143] & 7,
      pitchEnvelope: {
  ...
  static createSingleVoiceDump(patch: Patch, deviceId: number = 0): Uint8Array {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      vced[s + 14] = op.lfoAmpModSens; vced[s + 15] = op.velocitySens;
      vced[s + 16] = op.volume; vced[s + 17] = op.oscMode; vced[s + 18] = op.freqCoarse; vced[s + 19] = op.freqFine;
      vced[s + 20] = op.detune;
    }
    ...
    vced[134] = (patch.algorithm - 1) & 31;
    vced[135] = patch.feedback & 7;
    vced[136] = patch.oscKeySync ? 1 : 0;
    vced[137] = patch.lfoSpeed;
    vced[138] = patch.lfoDelay;
    vced[139] = patch.lfoPitchModDepth;
    vced[140] = patch.lfoAmpModDepth;
    vced[141] = patch.lfoSync ? 1 : 0;
    vced[142] = patch.lfoWaveform & 7;
    vced[143] = patch.lfoPitchModSens & 7;
    vced[144] = patch.transpose;

    const encoder = new TextEncoder();
    const nameStr = (patch.name + "          ").substring(0, 10).toUpperCase();
    const nameBytes = encoder.encode(nameStr);
    for (let i = 0; i < 10; i++) vced[145 + i] = nameBytes[i];

    const msg = new Uint8Array(163);
    msg[0] = 0xF0;
    msg[1] = 0x43;
    msg[2] = 0x00; // Hardcode to 0 as channel is not the issue
    msg[3] = 0x00;
    msg[4] = 0x01; // Byte count 155 (0x01 0x1B in 7-bit)
    msg[5] = 0x1B;
    msg.set(vced, 6);
